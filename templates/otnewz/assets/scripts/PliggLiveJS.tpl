...
function to_html(data) {
	...
	if (data.type == 'problem')
		html += '<td class="liveAction">'+data.type+'</td>';
	else if (data.type == 'Submission')
		html += '<td class="liveAction"><a href="' + my_pligg_base + '/live_unpublished.php">'+data.type+'</a></td>';
	else if (data.type == 'Published')
		html += '<td class="liveAction"><a href="' + my_pligg_base + '/live_published.php">'+data.type+'</a></td>';	
	else if (data.type == 'Comment')
		html += '<td class="liveAction"><a href="' + my_pligg_base + '/live_comments.php">'+data.type+'</a></td>';	
	else
		html += '<td class="liveAction">'+data.type+'</td>';

	html += '<td class="liveVotes">'+data.votes+'</td>';
	html += '<td class="liveStory"><a href="' + my_pligg_base + '/story.php?id='+data.link+'">'+data.title+'</a></td>';
	if (data.type == 'problem')
		html += '<td class="liveUser">'+data.who+'</td>';
	else if (data.uid > 0) 
		html += '<td class="liveUser"><a href="' + my_pligg_base + '/user.php?login='+data.who+'">'+data.who+'</a></td>';
	else 
		html += '<td class="liveUser">{/literal}{if $isAdmin eq 1}'+data.who+'{else}Anonymous{/if}{literal}</td>';
	html += '<td class="liveState">'+data.status+'</td>';
	return html;
}
{/literal}
